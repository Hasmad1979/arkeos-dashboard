...
st.markdown("""
    ...
    .status-red { border-left: 5px solid #ff4b4b !important; color: #ff4b4b !important; }
    .status-green { border-left: 5px solid #00cc96 !important; color: #00cc96 !important; }
    </style>
...
def load_data_v2():
    ...
    # Identification des colonnes (ajout du client)
...
if df.empty:
    st.error("Données introuvables. Vérifiez le fichier et les colonnes.")
else:
    ...
    # --- AFFICHAGE PRO DES KPI ---
    rdr_color = "status-red" if rdr > 20 else "status-green"
    fttr_color = "status-green" if fttr > 60 else "status-red"

    c1, c2, c3, c4 = st.columns(4)
    with c1:
        st.markdown(f'<div class="metric-card"><div class="metric-label">Interventions</div><div class="metric-value">{total}</div></div>', unsafe_allow_html=True)
    with c2:
        st.markdown(f'<div class="metric-card {rdr_color}"><div class="metric-label">RDR % (>20%=⚠️)</div><div class="metric-value">{rdr:.1f}%</div></div>', unsafe_allow_html=True)
    with c3:
        st.markdown(f'<div class="metric-card {fttr_color}"><div class="metric-label">FTTR % (<60%=⚠️)</div><div class="metric-value">{fttr:.1f}%</div></div>', unsafe_allow_html=True)
    with c4:
        st.markdown(f'<div class="metric-card"><div class="metric-label">Total Repeats</div><div class="metric-value">{int(reps)}</div></div>', unsafe_allow_html=True)
    ...
    with col_left:
        st.subheader("⚠️ Top 10 Actifs (Repeats)")
